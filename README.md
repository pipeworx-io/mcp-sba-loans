...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/sba_loan_lookup \
  -H 'Content-Type: application/json' \
  -d '{"borrower_name":"Ameripro Construction","state":"PA"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/sba_loan_lookup`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
